...
- Wan 超分（2026-09-17，待部署）：`alibaba/wan-3.0` 与 `alibaba/wan-3.0-prime` 复用后台超分配置，1080P 可选 480P / 720P 源视频；未启用或低分辨率请求保持原样。服务端使用当前渠道凭据读取原片，暂存为短期签名地址供既有 BytePlus VOD 工作流拉取，完成后复用成片交付、管理员原片保留及清理流程。相关本地测试通过；未提交付费生成，尚未做生产端到端验收。
...
- MiniMax H3 视频能力：`minimax-h3` Task Plugin 与 `minimax-h3-fl2va` Playground 已支持文生、首帧、首尾帧和 Ref2VA 参考内容生视频。参考模式支持最多 2 张 JPEG / PNG / WebP 图片和 1 个不超过 64 MiB 的 MP4 / MOV 视频，前端沿用参考素材与 `@` 引用交互。用户已通过公网完成参考图、参考视频及二者混合的真实生成，制品可播放和下载；有声任务与实际扣费已完成数据库对账，失败退款仍待验证。
...
- 下一主 Milestone：补验 H3 失败退款并落实 ComfyUI 参考素材 TTL 清理，再推进 Seedance 本地参考视频验收与 Demo 收口。
...
9. MiniMax H3 文生、首帧、首尾帧、参考内容与有声生成均已完成公网验收，实际扣费记录也已对账；失败退款、Windows Firewall 边界及模型许可证和商业使用条件仍需单独核对。
10. H3 参考视频原始音频当前不作为模型参考输入。视频会复制到远端 ComfyUI `input` 目录，仍需建立只清理 `minimax-h3-*-reference-video.*` 的安全 TTL 任务，避免磁盘累积与素材长期留存。
...
1. 为远端 ComfyUI `input` 目录配置 H3 参考视频安全 TTL 清理，并继续保持 ComfyUI `8888` 仅通过受控 Tailscale 私网访问。
2. 补验 H3 失败退款，确认失败任务不会形成最终扣费且预扣额度能够正确退回。
