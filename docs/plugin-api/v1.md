...
A submit descriptor may include one `prepareRequest` for a provider upload or other prerequisite immediately before the main request. It cannot contain another `prepareRequest`, cannot be `credentialless`, and may use only `GET`, `HEAD`, or `POST`. Its URL follows the same channel base URL and `allowedHosts` validation as the main descriptor; its body is JSON or multipart. A transport failure aborts the submit. A non-2xx response is returned through the normal upstream status path, so the main request is not sent. Input-related `400`, `413`, `415`, and `422` responses are treated as non-retryable request errors; other statuses follow the existing retry rules.
