...
it('retains child-local filter state after collapsing and expanding', async () => {
  const user = userEvent.setup()
  render(
    <DataTableMobileFilterPanel actions={<Button>Apply</Button>}>
      <LocalDraftFilter />
    </DataTableMobileFilterPanel>
  )

  await user.type(screen.getByRole('textbox', { name: 'Local draft' }), 'gpt')
  await user.click(screen.getByRole('button', { name: 'Collapse' }))
  expect(
    screen.queryByRole('textbox', { name: 'Local draft' })
  ).not.toBeInTheDocument()
  await user.click(screen.getByRole('button', { name: 'Expand' }))

  expect(screen.getByRole('textbox', { name: 'Local draft' })).toHaveValue(
    'gpt'
  )
})
