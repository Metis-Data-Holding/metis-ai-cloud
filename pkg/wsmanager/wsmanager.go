...
func CloseChannels(channelIDs []int, reason string) int {
	...
	semaphore := make(chan struct{}, maxCloseConcurrency)
		...
		wg.Go(func() {
			defer func() {
				...
				if recovered := recover(); recovered != nil {
					common.SysLog(fmt.Sprintf("websocket close callback panic: channel_id=%d, kind=%s, panic_type=%T", e.channelID, e.kind, recovered))
				}
			}()
			e.close(reason)
		})
