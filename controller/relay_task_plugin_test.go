...
func TestExecuteTaskSubmissionRetainsImmediateResultDuringSuperResolution(t *testing.T) {
	events := make([]string, 0, 3)
	database, _ := openTaskDialectDatabase(t, &model.Task{}, &model.User{}, &model.Channel{})
	previousDB := model.DB
	model.DB = database
	...
	retainFalse := false
	c.Set(pluginruntime.ContextKeyPinnedRoute, pluginruntime.PinnedRoute{
		Plugin: &pluginruntime.LoadedPlugin{Meta: pluginruntime.Meta{Key: "doubao"}},
		Route:  pluginruntime.Route{Method: http.MethodPost, Path: "/doubao/jobs", Type: pluginruntime.RouteTypeSubmit, RetainResult: &retainFalse},
	})
	info := taskSubmissionRelayInfo(&taskSubmissionTestBilling{events: &events})
	outcome, taskErr := executeTaskSubmissionWith(c, info, func(*gin.Context, *relaycommon.RelayInfo) (*relay.TaskSubmitResult, *dto.TaskError) {
		// The retry loop clears stale snapshots before each submit attempt.
		c.Set("video_super_resolution.snapshot", service.VideoSuperResolutionSnapshot{SourceResolution: "720p", TargetResolution: "1080p"})
		_, enabled := service.GetVideoSuperResolutionSnapshot(c)
		require.True(t, enabled)
		return &relay.TaskSubmitResult{
			UpstreamTaskID: "task_public",
			Platform:       constant.TaskPlatform("doubao"),
			TaskData:       []byte(`{"data":{"id":"upstream-private"}}`),
			Immediate:      &relaycommon.TaskInfo{Status: model.TaskStatusSuccess, Url: "https://cdn.example/original.mp4"},
		}, nil
	})
	require.Nil(t, taskErr)
	require.NotNil(t, outcome)
	assert.Equal(t, model.TaskStatus(model.TaskStatusInProgress), outcome.Task.Status)
	assert.False(t, outcome.Task.PrivateData.ResultDiscarded)
	require.NotNil(t, outcome.Task.PrivateData.SuperResolution)
	assert.Equal(t, "upload_pending", outcome.Task.PrivateData.SuperResolution.Phase)
	assert.Equal(t, "https://cdn.example/original.mp4", outcome.Task.PrivateData.SuperResolution.OriginalURL)
	assert.JSONEq(t, `{"status":"processing"}`, string(outcome.Task.Data))

	var stored model.Task
	require.NoError(t, database.Where("task_id = ?", "task_public").First(&stored).Error)
	assert.True(t, stored.ResultRetrievable())
	assert.JSONEq(t, `{"status":"processing"}`, string(stored.Data))
}
