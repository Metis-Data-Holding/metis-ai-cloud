...
func TestVLLMStatusDoesNotExposeRawPrometheusMetrics(t *testing.T) {
	service.InitHttpClient()
	const sensitiveMarker = "metrics-secret-marker"
	upstream := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		switch r.URL.Path {
		case "/health":
			w.WriteHeader(http.StatusOK)
		...
		case "/v1/models":
			_, _ = w.Write([]byte(`{"data":[]}`))
		case "/metrics":
			_, _ = w.Write([]byte("# HELP vllm:num_requests_running active requests\n# TYPE vllm:num_requests_running gauge\nvllm:num_requests_running{engine=\"0\",token=\"" + sensitiveMarker + "\"} 0\n# HELP unrelated_metric " + sensitiveMarker + "\n# TYPE unrelated_metric gauge\nunrelated_metric{token=\"" + sensitiveMarker + "\"} 1\n"))
		default:
			t.Errorf("unexpected path: %s", r.URL.Path)
			w.WriteHeader(http.StatusNotFound)
		}
	...
	require.NoError(t, err)
	assert.Contains(t, string(encoded), `"engine":"0"`)
	assert.NotContains(t, string(encoded), `"token"`)
	assert.NotContains(t, string(encoded), `"raw_metrics"`)
	assert.NotContains(t, string(encoded), sensitiveMarker)
}
