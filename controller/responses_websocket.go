...
const maxResponsesWSConnectionsPerUser = 32

type responsesWSConnectionTracker struct {
	mutex  sync.Mutex
	active map[int]int
}

func (t *responsesWSConnectionTracker) acquire(userID int) bool {
	t.mutex.Lock()
	...
	if t.active[userID] >= maxResponsesWSConnectionsPerUser {
		return false
	}
	t.active[userID]++
	return true
...
func (t *responsesWSConnectionTracker) release(userID int) {
	t.mutex.Lock()
	defer t.mutex.Unlock()

	if t.active[userID] <= 1 {
...
var responsesWSConnectionLimit responsesWSConnectionTracker
...
func ResponsesWebSocket(c *gin.Context) {
	userID := c.GetInt("id")
	if !responsesWSConnectionLimit.acquire(userID) {
		c.AbortWithStatus(http.StatusTooManyRequests)
		return
	}
	defer responsesWSConnectionLimit.release(userID)
