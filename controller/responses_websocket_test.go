...
func TestResponsesWebSocketConnectionLimitTracksActiveUsers(t *testing.T) {
	resetResponsesWSConnectionLimit := func() {
		responsesWSConnectionLimit.mutex.Lock()
		defer responsesWSConnectionLimit.mutex.Unlock()
		responsesWSConnectionLimit.active = nil
	}
	resetResponsesWSConnectionLimit()
	t.Cleanup(resetResponsesWSConnectionLimit)

	const userID = 8000001
	const otherUserID = 8000002
	request := func(userID int) *httptest.ResponseRecorder {
		...
		engine.ServeHTTP(recorder, httptest.NewRequest(http.MethodGet, "/v1/responses", nil))
		return recorder
	}

	// A failed handshake must release the slot so the user can fill the limit.
	failedHandshake := request(userID)
	require.Equal(t, http.StatusBadRequest, failedHandshake.Code)
	for range maxResponsesWSConnectionsPerUser {
		require.True(t, responsesWSConnectionLimit.acquire(userID))
	}

	// The 33rd connection for one user is rejected before the upgrader, while
	// another user can still attempt a connection independently.
	limited := request(userID)
	assert.Equal(t, http.StatusTooManyRequests, limited.Code)
	otherUser := request(otherUserID)
	assert.Equal(t, http.StatusBadRequest, otherUser.Code)

	// Releasing one active slot allows a new handshake, and its failed upgrade
	// also releases the slot for another connection.
	responsesWSConnectionLimit.release(userID)
	rebuilt := request(userID)
	assert.Equal(t, http.StatusBadRequest, rebuilt.Code)
	require.True(t, responsesWSConnectionLimit.acquire(userID))
}
