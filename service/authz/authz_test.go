...
func newAuthzTestDB(t *testing.T) *gorm.DB {
	t.Helper()
	return newAuthzTestDBWithDialector(t, sqlite.Open(":memory:"))
}

func newAuthzTestDBWithDialector(t *testing.T, dialector gorm.Dialector) *gorm.DB {
	t.Helper()
...
func TestInitRemovesLegacyGlobalBuiltInRoleGrants(t *testing.T) {
	tests := []struct {
		name      string
		dsnEnv    string
		...
		{name: "mysql", dsnEnv: "TEST_AUTHZ_MYSQL_DSN", dialector: mysql.Open},
		{name: "postgres", dsnEnv: "TEST_AUTHZ_POSTGRES_DSN", dialector: postgres.Open},
	}
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			db := newAuthzTestDBWithDialector(t, test.dialector(dsn))
			if test.dsnEnv != "" {
				t.Cleanup(func() {
					_ = db.Migrator().DropTable(&model.CasbinRule{}, &model.AuthzRole{})
				})
			}

			stale := model.CasbinRule{
				Ptype: "p",
			...
			var count int64
			require.NoError(t, db.Model(&model.CasbinRule{}).Where("id = ?", stale.Id).Count(&count).Error)
			assert.Zero(t, count)
