		...
		for _, tc := range []struct {
			...
			t.Run(kind+"/"+tc.model+"/"+tc.size, func(t *testing.T) {
				...
				if err != nil && tc.reject {
					require.ErrorContains(t, err, "resolution must be one of")
					return
				}
				require.NoError(t, err)
				encoded, err = common.Marshal(value)
...
func TestDoubaoSeedanceUsageFacts(t *testing.T) {
	...
	families := []struct {
		...
		{[]string{v20, "dreamina-seedance-2-0-260128"}, []string{"480p", "720p", "1080p", "4k"}, []string{"resolution", "tokens", "video_input"}},
		{[]string{fast20, mini20, "dreamina-seedance-2-0-fast-260128"}, []string{"480p", "720p"}, []string{"resolution", "tokens", "video_input"}},
