...
const VIDEO_MODELS = {
  ...
  "dreamina-seedance-2-0-260128": { resolutions: ["480p", "720p", "1080p", "4k"], videoInput: true },
  "dreamina-seedance-2-0-fast-260128": { resolutions: ["480p", "720p"], videoInput: true },
};
...
export const meta = {
  apiVersion: 1,
  key: "doubao",
  name: "Doubao / Dreamina",
  icon: "Doubao.Color",
  description: {
    en: "Volcengine Doubao and BytePlus Dreamina Seedance video generation, and Seedream image generation",
    zh: "火山引擎豆包与 BytePlus Dreamina Seedance 视频生成，以及 Seedream 图片生成",
  },
  version: "1.1.1",
...
function videoResolution(ctx) {
  ...
  if (!raw) return profile.resolutions.includes("1080p") ? "1080p" : profile.resolutions[profile.resolutions.length - 1];
...
export function buildSubmitRequest(ctx) {
  ...
  if (Object.prototype.hasOwnProperty.call(req, "resolution") || trimmed(metadata.resolution) || trimmed(req.size)) body.resolution = resolution;
  const imageContent = [];
...
export function extractUsage(ctx) {
  ...
  if (ctx.usagePurpose === "billing_ratios") {
    const ratio = videoInputRatio(ctx.upstreamModel || ctx.model, videoResolution(ctx), metadata.content);
  ...
  if (profile.videoInput) facts.video_input = video ? "video" : "none";
...
export const protocols = {
  openai_responses: {
    decodeRequest: function (ctx) {
      ...
      if (Object.prototype.hasOwnProperty.call(req, "resolution")) metadata.resolution = normalizeResolution(req.resolution);
